...
import { PositionModalComponent } from './modal/modal.component';
...
export class PositionComponent implements OnInit, AfterViewInit, OnDestroy
{
  ...
  ngOnInit(): void
  {
    ...
    // Get the pagination
    this._positionService.pagination$
    .pipe(takeUntil(this._unsubscribeAll))
    .subscribe((pagination: PositionPagination) =>
    {
    ...
    .pipe(
        ...
        {
            this.isLoading = true;
            return this._positionService.getPosition(0, 10, 'name', 'asc', query);
        }),
        map(() =>
        {
            this.selection.clear();
            this.positions$.subscribe(data => { data.forEach(row => this.selection.deselect(row)); });
    ...
    if ( this._sort && this._paginator )
    {
      ...
      this._sort.sortChange
        ...
        merge(this._sort.sortChange, this._paginator.page).pipe(
          ...
          {
              this.isLoading = true;
              return this._positionService.getPosition(this._paginator.pageIndex, this._paginator.pageSize, this._sort.active, this._sort.direction);
          }),
          map(() =>
          {
              this.selection.clear();
  ...
  masterToggle() 
  {
    ...
    {
      this.showDelete = true;
      this.positions$.subscribe(data => { data.forEach(row => this.selection.select(row)); });
    }
  }
  ...
  createPosition(): void
  {
    const dialogRef = this._matDialog.open(PositionModalComponent, {
      autoFocus: false,
      data     : {
          position: {},
      },
   ...
   * Edit User
   */
  editPosition(data: Position): void
  {
    this._matDialog.open(PositionModalComponent, {
      autoFocus: false,
      data     : {
        position: data,
      },
  ...
  deleteUser(data: Position): void
  {
    try {
      ...
      confirmation.afterClosed().subscribe((result) =>
      {
          ...
          {
              // Delete
              this._positionService.delete(data.positionId).subscribe(() =>
              {
                this.positions$.subscribe(data => {
                  if(data.length == 0)
                  {
    ...
    confirmation.afterClosed().subscribe((result) =>
      {
          if ( result === 'confirmed' )
          {
            this.selection.selected.forEach(s => {
              ...
              // Delete
              this._positionService.delete(s.positionId).subscribe(() =>
              {
                if(this.selection.selected.length == 0) 
                {
                  this.showDelete = false;
